...
namespace ModIOBrowser.Implementation
{
    ...
    internal static class Utility
    {
        ...
        public static string GenerateHumanReadableDownloadSpeedStringFromBytesPerSecond(long bytesPerSecond)
        {
            if(bytesPerSecond / 1048576 > 0)
            {
                return (bytesPerSecond / 1048576).ToString("0.0") + "MB";
            } 
            else
            {
                return (bytesPerSecond / 1024).ToString("0.0") + "KB";
            }
        }
        ...
        {
            if(A.fileSizeInBytes > B.fileSizeInBytes)
            {
                return -1;
            }
            if(A.fileSizeInBytes < B.fileSizeInBytes)
