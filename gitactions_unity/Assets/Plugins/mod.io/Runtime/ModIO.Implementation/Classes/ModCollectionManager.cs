...
#region Syncing User Fields
        ...
        public static void ClearUserData()
        {
            ...
            Registry.existingUsers.Remove(UserData.instance.userObject.username);
        ...
        public static void AddUserToRegistry(UserObject user)
        {
            ...
            if(!Registry.existingUsers.ContainsKey(user.username))
            {
                Registry.existingUsers.Add(user.username, newUser);
            }
        ...
        public static async Task<Result> FetchUpdates()
        {
            ...
            // set the user we're checking for
        ...
        static async Task<Result> RunFirstSyncForTheSession(string user)
        {
            Result result = ResultBuilder.Success;
        ...
        /// <param name="user">username of the user</param>
        /// <returns>The Result of all the operations (returns at the first non-success result)</returns>
        static async Task<Result> RunRepeatedSyncForTheSession(string user)
        {
            Result result = ResultBuilder.Success;
            ...
            if(userResultAnd.result.Succeeded())
            {
                ...
                if(userResultAnd.value != null)
                {
                    foreach(UserEventObject userEvent in userResultAnd.value)
                    {
                        ...
                        {
                            AddModToUserSubscriptions(new ModId(userEvent.mod_id), user);
                        }
        ...
        public static void AddModToUserSubscriptions(ModId modId, [CanBeNull] string user = null,
                                                                           ...
                                                                           string directory)
        {
            ...
            mod.subscribedUsers = new List<string>();

            foreach(string user in Registry.existingUsers.Keys)
            {
...
#region public Checks &Utility
        ...
        public static bool DoesUserExist([CanBeNull] string user = null)
        {
            if(user == null)
            {
                ...
                user = UserData.instance.userObject.username;

                if(user == null)
                {
                    Logger.Log(LogLevel.Error, "The current user has not been authenticated "
                                                   + "properly (The UserObject username is null).");
                    return false;
                }
            }
        ...
        /// </summary>
        /// <param name="user">returns the correct user key for the registry (if user is null it will return the current user)</param>
        /// <returns></returns>
        public static string GetUserKey([CanBeNull] string user = null)
        {
            return user ?? UserData.instance.userObject.username;
        }
