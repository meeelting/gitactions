...
#region Request Handling
                ...
                */
                ...
                switch(requestTemplate.requestResponseType)
                {
                    case WebRequestResponseType.Text:
                        ...
                        serializedResponse = ExtractTextResponse(webRequest.downloadHandler,
                                                                 out internalResponse.result);
                        break;
                    case WebRequestResponseType.Texture:
                        await WaitForTurnToSend(webRequest);
                        textureResponse = ExtractTextureResponse(webRequest.downloadHandler,
                                                                 out internalResponse.result);
                        break;
                    case WebRequestResponseType.File:
                ...
                switch(responseCode)
                {
                    ...
                    case ResponseCodeType.Succeeded:
                        ...
                        {
                            logTitle =
                                $"[{internalResponse.result.code}:{internalResponse.result.code_api}] PROCESSING ERROR\nThe WebRequest appeared to succeed but the response failed to be processed\n\n";
                        }
        ...
        static string ExtractTextResponse(DownloadHandler downloadHandler, out Result result)
        {
        ...
        static Texture2D ExtractTextureResponse(DownloadHandler downloadHandler, out Result result)
        {
                      ...
                      && liveProgressHandles.Contains(progressHandle))
                {
                    ...
                    if (samplesTotalSize != 0)
                    {
                        progressHandle.BytesPerSecond =
                            (long)(samplesTotalSize / (ulong)sampleTimeInMilliseconds);
                    }
